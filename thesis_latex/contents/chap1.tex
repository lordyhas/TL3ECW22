...
\textcolor{cyan}{\chapter{Concepts de base et état de connaissances}\label{chap:concept}}
	...
	%\subsection*{Invarience de transformation}
	...
	\section{Les Éléments d'optimisation numérique} \label{sec:optimisation}
		...
		\begin{wrapfigure}{r}{0.4\textwidth}
			\myfloatalign{\includegraphics[width=7cm]{images/convex_function_graph-2}}
	...
	%L'objectif est d'estimer la fonction ${\displaystyle f(X_{i},w )}$ qui correspond le mieux aux données.\\Pour effectuer une analyse de régression, la forme de la fonction $f$ doit être spécifié. Parfois, la forme de cette fonction est basée sur l'information de la relation entre $Y_{i}$ et $X_{i}$. Si ces informations ne sont pas disponibles, un cas souple ou pratique pour $f$ est choisi. Par exemple, une simple régression univariée peut proposer
	...
	%Cette relation est \textbf{modélisée} par un terme de perturbation ou une variable d'erreur $\epsilon$ : une variable aléatoire non observée qui ajoute du "bruit" à la relation linéaire entre la variable dépendante et les régresseurs 
	\begin{equation}
	...
	%\paragraph*{régression linéaire multiple}La régression linéaire multiple est une généralisation de la régression linéaire simple au cas de plus d'une variable indépendante, et un cas particulier des modèles linéaires généraux, limités à une variable dépendante.
	...
	\begin{wrapfigure}{r}{0.3\textwidth}
		\myfloatalign{\includegraphics[width=6cm]{images/classification_minimisation}}
	...
	La forme de la fonction pour toutes les entrées possibles est une forme en S (figure \ref{fig:sigmoid-graph}) de zéro jusqu'à 0,5 à 1,0 \cite{krizhevsky2012imagenet}. %Pendant longtemps, jusqu'au début des années 1990, c'était l'activation par défaut utilisée sur les réseaux de neurones .
